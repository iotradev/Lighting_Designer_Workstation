...
class ProjectWidget(QWidget):
    ...
    def refresh(self):
        ...
        for p in projects:
            ...
            item.setToolTip(f": {p.get('venue', '')}\n: {p.get('modified', '')}")
            self.project_list.addItem(item)
        self.info_label.setText(f"?{len(projects)} "?)
    ...
    def _context_menu(self, pos):
        ...
        if not item:
            return
        menu = QMenu(self)
        menu.addAction("", lambda: self._on_double_click(item))
        menu.addAction("", lambda: self._open_in_explorer(item))
        menu.addSeparator()
        menu.addAction("", lambda: self._delete_project(item))
