...
class BaseToolWindow(QMainWindow):
    ...
    def _init_menu_bar(self):
        ...
        self._add_action(file_menu, "(&N)", self._on_new_project, QKeySequence.StandardKey.New)
        self._add_action(file_menu, "(&O)...", self._on_open_project, QKeySequence.StandardKey.Open)
        file_menu.addSeparator()
        self._add_action(file_menu, "(&S)", self._on_save_project, QKeySequence.StandardKey.Save)
        self._add_action(file_menu, "&A)...", self._on_save_as, QKeySequence("Ctrl+Shift+S"))
        file_menu.addSeparator()

        # 
        self.recent_menu = file_menu.addMenu("&R)")
        self._update_recent_menu()

        file_menu.addSeparator()
        self._add_action(file_menu, "&Q)", self.close, QKeySequence("Alt+F4"))

        # 
        edit_menu = menubar.addMenu("(&E)")
        self._add_action(edit_menu, "", self._stub(""), QKeySequence.StandardKey.Undo)
        self._add_action(edit_menu, "", self._stub(""), QKeySequence.StandardKey.Redo)
        edit_menu.addSeparator()
        self._add_action(edit_menu, "", self._stub(""), QKeySequence.StandardKey.Cut)
        self._add_action(edit_menu, "", self._stub(""), QKeySequence.StandardKey.Copy)
        self._add_action(edit_menu, "", self._stub(""), QKeySequence.StandardKey.Paste)

        # 
        view_menu = menubar.addMenu("(&V)")
        self._add_action(view_menu, "", self._toggle_log_panel, QKeySequence("Ctrl+L"))
        self._add_action(view_menu, "", self._toggle_project_panel, QKeySequence("Ctrl+P"))
        view_menu.addSeparator()
        self._add_action(view_menu, "", lambda: self._switch_theme("dark"))
        self._add_action(view_menu, "", lambda: self._switch_theme("light"))
        ...
        self._add_action(help_menu, "", self._show_about)
        self._add_action(help_menu, "", self._show_shortcuts)
    ...
    def _on_save_project(self):
        ...
        if self.project_mgr.current_project:
            ...
            QTimer.singleShot(3000, lambda: self.status_ready.setText(""))
        else:
            self._on_save_as()
    ...
    def _update_project_status(self, name):
        """"""
        self.status_project.setText(f": {name}")
    ...
    def _show_about(self):
        QMessageBox.about(self, f" {self.tool_title}",
            ...
            f"<p>: {self.version}</p>"
            f"<p>Lighting Designer Workstation - ?/p>"
            f"<p>?/p>")
    ...
    def _show_shortcuts(self):
        ...
        <tr><td><b>Ctrl+Shift+S</b></td><td>?/td></tr>
        <tr><td><b>Ctrl+L</b></td><td></td></tr>
        <tr><td><b>Ctrl+P</b></td><td>?/td></tr>
        <tr><td><b>F11</b></td><td></td></tr>
        <tr><td><b>Alt+F4</b></td><td>?/td></tr>
        </table>
        """
